...
function msg_receive(msg) {
	...
	var h = d.getHours();
	var m = d.getMinutes();
	return "<li class='msg_receive'><div class='chat-content'><div class='box bg-light-info'>" + msg + "</div></div><div class='chat-time d-inline-block text-end fs-2 font-weight-medium'>" + h + ":" + m + "</div></li>";
}
...
function msg_sent(msg) {
	...
	var h = d.getHours();
	var m = d.getMinutes();
	return "<li class='odd msg_sent'><div class='chat-content'><div class='box bg-light-info'>" + msg + "</div><br></div><div class='chat-time d-inline-block text-end fs-2 font-weight-medium'>" + h + ":" + m + "</div></li>";
}
...
$('.message-type-box').on('keydown', function(event) {
    if(event.key === 'Enter') {
        ...
        $messageHtml = '<li class="odd mt-4"> <div class="chat-content pl-3 d-inline-block text-end"> <div class="box mb-2 d-inline-block text-dark message font-weight-medium fs-3 bg-light-inverse">' + chatMessageValue + '<br> </div></div><div class="chat-time d-inline-block text-end fs-2 font-weight-medium">' +  time +'</div> </li>';
